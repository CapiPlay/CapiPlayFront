...
const Input = ({ placeholder, value, onChange, onClick, required, type, enable, name, onKeyDown }) => {
    ...
    return (
        <div className="input__tag__group">
            <input
                className="input__tag__control"
                type={inputType}
                value={value}
            ...
            />
            <label className={`input__tag__label__input ${hasValue ? "input__tag__active" : ""}`} style={labelColor}>
                {placeholder}
            </label>
        </div>
    )
