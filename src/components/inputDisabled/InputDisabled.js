...
const InputDisabled = ({ placeholder, value, onChange, onClick, required, type, enable, name }) => {
  ...
  return (
    <div className="input__group__disabled">
      <input
        className="input__control__disabled"
        type={inputType}
        value={value}
        ...
        disabled
      />
      <label className={`label__input__disabled ${hasValue ? "active" : ""}`} style={labelColor}>
        {placeholder}
      </label>
    </div>
  )
